...
export function SystemDiagram() {
  return (
    <Frame title="Figure 1 — System architecture: SPA over a decoupled REST API, with a WebSocket channel for real-time" height={400}>
      {/* client */}
      ...
      <text x="457" y="152" fill="#a78bfa" fontSize="10.5" fontFamily="monospace">WebSocket</text>
      <text x="457" y="166" fill="#525960" fontSize="9.5" fontFamily="monospace">stock · chat</text>
      ...
      <text x="463" y="269" textAnchor="middle" fill="#c4b5fd" fontSize="10.5" fontFamily="monospace">WebSocket server</text>
      ...
      <text x="380" y="374" textAnchor="middle" fill="#f1f5f9" fontSize="14" fontWeight="700">MySQL — every row scoped to a legal entity</text>
    </Frame>
  );
}
...
export function DataModelDiagram() {
  ...
  return (
    <Frame title="Figure 2 — Data model: entity attribution is carried on every stock row and every movement" height={330}>
      {t(40, 30, "legal_entity", ["id", "name", "tax_id"], "#00ff88")}
      {t(289, 30, "product", ["id", "sku", "name"])}
      {t(538, 30, "user", ["id", "name", "role"], "#fbbf24")}

      {t(40, 178, "stock", ["→ entity_id", "→ product_id", "quantity"], "#00d4ff")}
      {t(330, 178, "stock_movement", ["→ entity_id", "→ product_id", "→ user_id", "type · qty · at"], "#a78bfa")}

      {/* relations */}
      <line x1="131" y1="122" x2="131" y2="176" stroke="#00ff88" strokeWidth="1.2" markerEnd="url(#arw)" opacity="0.6" />
      ...
      <text x="380" y="308" textAnchor="middle" fill="#525960" fontSize="11" fontStyle="italic">
        Queries scope to one entity, or roll up across all three — from the same source of truth.
      </text>
    </Frame>
  );
}
...
export function TransferFlowDiagram() {
  ...
  return (
    <Frame title="Figure 3 — Transfer between legal entities: one transaction, so the two ledgers cannot disagree" height={210}>
      {step(20, "Operator", "records transfer", "#00d4ff")}
      {step(215, "API", "validates + scopes", "#00d4ff")}
      {step(410, "Transaction", "debit A · credit B", "#00ff88")}
      {step(605, "Broadcast", "WebSocket", "#a78bfa")}
      ...
      <text x="380" y="190" textAnchor="middle" fill="#525960" fontSize="11">
        This is the failure the spreadsheets had — rebuilding it in software would have defeated the purpose.
      </text>
    </Frame>
  );
}
